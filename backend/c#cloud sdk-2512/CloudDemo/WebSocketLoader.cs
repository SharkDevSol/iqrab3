...
namespace CloudDemo
{
    public class WebSocketLoader
    {
        ...
        public static struct_userlist[] str_userlist = new struct_userlist[20000];
                ...
                if(string.IsNullOrEmpty(cmd)==false) //client active send data
                {
                    ...
                    switch (cmd)
                    {
                        ...
                        case "reg":
                            ...
                            //////if refuse this machine to connect to server,the reason will display on screen
                             //string usernameutf8 = LogHelper.gb2312_utf8("余额不足,禁止使用");
                            // string usernameutf8 = LogHelper.gb2312_utf8("Insufficient balance!");
                            // strRespone = "{\"ret\":\"reg\",\"result\":false,\"reason\":\"" + usernameutf8 + "\",\"cloudtime\":\"" + server.ToString() + "\"}";


                            //////if you don't want to auto send the new add user to server ,then send:
                            // strRespone = "{\"ret\":\"reg\",\"result\":true,\"nosenduser\":true,\"cloudtime\":\"" + server.ToString() + "\"}";
                        ...
                        case "sendlog":        
                            ...
                            foreach (var ss in attRecords)
                            {
                               ...
                               if (!string.IsNullOrEmpty(ss.Value<string>("image")))
                               {
                                   var logimage = ss.Value<string>("image");
                                   byte[] lograwjpg = Convert.FromBase64String(logimage);
                                   System.IO.File.WriteAllBytes(@"C:\\EnrollPhoto\log\" + enrollid.ToString() + "-" + timelog.ToString("yyyyMMddhhmmss") + ".jpg", lograwjpg);
                               }
                            }
                             ...
                             strRespone = strRespone + ",\"access\":1"; //add the server mode to indetify the door open or not:access 1 can open, 0 can not open
                             strRespone = strRespone + ",\"message\":\"this is the reson of access\""; //if need will display at notice                        
                        ...
                        case "senduser":
                             ...
                             if (backupnum >= 20 && backupnum < 42) //is fp
                             {
                            ...
                            {
                                var pwddata= jsonMsg.Value<Int64>("record");
                            }      
							else if(backupnum==50) //aiface photo base 64
							{
							    var photobase64 =jsonMsg.Value<string>("record");
                                byte[] rawjpg = Convert.FromBase64String(photobase64);
                                System.IO.File.WriteAllBytes(@"C:\\EnrollPhoto\" + "LF" + enrollid.ToString().PadLeft(8, '0') + ".jpg", rawjpg);
                                Console.WriteLine("pictures saving " + @"C:\EnrollPhoto\" + "LF" + enrollid.ToString().PadLeft(8, '0') + ".jpg");
							}
                            strRespone = "{\"ret\":\"senduser\",\"result\":true,\"enrollid\":" + enrollid +",\"backupnum\":"+ backupnum + "}";
                            session.Send(strRespone);
                            break;
                        case "sendqrcode":
                            var qrcode = jsonMsg.Value<string>("record");
                            Console.WriteLine("final decode json====>record:" + qrcode);
                            strRespone = "{\"ret\":\"sendqrcode\",\"result\":true";
                            strRespone = strRespone + ",\"access\":1"; //if need :access 1 can open the door, 0 can not open
                            strRespone = strRespone + ",\"enrollid\":111"; //[option] //for save log and send wg out
                            strRespone = strRespone + ",\"username\":\"TEST QR\""; //[option]
                            strRespone = strRespone + ",\"message\":\"QR code\""; //[option]will display at notice 
                            strRespone = strRespone + ",\"voice\":\"QR code\""; //[option]will play the vioce 
                           // strRespone = strRespone + ",\"voice\":\"<error><id><name>二QR code\""; //[option] <error> will replace "dong dong"  <id>will replace "enrollid" <name> will replace "username" you input in json
                            strRespone = strRespone + "}";
                            session.Send(strRespone);
                            break;
                        case "sendpin":
                            var inputpin = jsonMsg.Value<string>("pin");  //this ping is string ,what user input.
                            var tempdatetime = jsonMsg.Value<string>("time"); //this is the time when user input.
                            Console.WriteLine("pin:" + inputpin);
                            Console.WriteLine("time:" + tempdatetime);
                            //////////////////////////////////////
                            strRespone = "{\"ret\":\"sendpin\",\"result\":true";
                            strRespone = strRespone + ",\"access\":1"; //if you want to open door.
                            strRespone = strRespone + ",\"message\":\"you are welcome\"";  //if you want to display some meesga at message label.
                            strRespone = strRespone + ",\"voice\":\"<id><name>verify ok\""; //if you want to say somthing ,but just english and chinese.
                            strRespone = strRespone + ",\"voiceindex\":1"; //if you want paly some wav what is download from "setvoice" cammand of the server.
                            strRespone = strRespone + ",\"name\":\"chingzou\""; //if you want to display name label 
                            strRespone = strRespone + "}";
                            session.Send(strRespone);
                            break;
                        default:
                    ...
                    switch(ret)
                    {
                        ...
                        case "getuserinfo":
                            ...
                            if(result==true)
                            {
                                ...
                                if (tmpuserinfo.backupnum >= 20 && tmpuserinfo.backupnum < 42) //is face
                                {
        ...
        public static void setuserinfo(string sn, int enrollid, string username, int backupnum, int admin, double carddata, string fpdata)
        {
           ...
           // string usernameutf8 =username;
            //////////////////////////////////////option end
            int verifymode = 0;
            if (backupnum >= 20 && backupnum < 42) //is face
                cmdstring = "{\"cmd\":\"setuserinfo\",\"enrollid\":" + enrollid + ",\"name\":\"" + usernameutf8 + "\",\"verifymode\":" + verifymode + ",\"backupnum\":" + backupnum + ",\"admin\":" + admin + ",\"record\":\"" + fpdata + "\"}";
            else if (backupnum >= 0 && backupnum < 10) //is fp
                cmdstring = "{\"cmd\":\"setuserinfo\",\"enrollid\":" + enrollid + ",\"name\":\"" + usernameutf8 + "\",\"verifymode\":" + verifymode + ",\"backupnum\":" + backupnum + ",\"admin\":" + admin + ",\"record\":\"" + fpdata + "\"}";
            else if (backupnum == 10) //password
                cmdstring = "{\"cmd\":\"setuserinfo\",\"enrollid\":" + enrollid + ",\"name\":\"" + usernameutf8 + "\",\"verifymode\":" + verifymode + ",\"backupnum\":" + backupnum + ",\"admin\":" + admin + ",\"record\":" + carddata + "}";
        ...
        public static void gettime(string sn)
        {
            WebSocketSession session;
            session = GetSessionByID(sn);
            if (session == null)
        ...
        public static void adduser(string sn, int enrollid, int backupnum, int admin, string username)
        {
            WebSocketSession session;
            session = GetSessionByID(sn);
            if (session == null)
		...
		public static void setuserprofile(string sn, int enrollid,string userprofile)
        {
            WebSocketSession session;
            session = GetSessionByID(sn);
            if (session == null)
                return;
            string userprofileutf8 = LogHelper.gb2312_utf8(userprofile);
            string cmdstring;
            cmdstring = "{\"cmd\":\"setuserprofile\",\"enrollid\":" + enrollid + ",\"profile\":\"" + userprofileutf8 + "\"}"; //if enrollid=0 ,is notice ,for machine alway display
            session.Send(cmdstring);
        }
        public static void keypad_onoff(string sn, bool onoff)
        {
            WebSocketSession session;
            session = GetSessionByID(sn);
            if (session == null)
            ...
            //if you want to open server-keypad funcfion, and machine reboot will keep this funcfion.
           // cmdstring = "{\"cmd\":\"keypad\",\"keep\":true}";
            cmdstring = "{\"cmd\":\"keypad\",\"keep\":" + onoff.ToString().ToLower() + "}"; 
            ////////////////////if you want to active keypad now or cancel
            /*
            if (onoff)
            cmdstring = "{\"cmd\":\"keypad\",\"on\":true,\"timeout\":100}";
            else
            cmdstring = "{\"cmd\":\"keypad\",\"on\":false}";
            */
            session.Send(cmdstring);
        }
		public static void getuserprofile(string sn, int enrollid)
        {
            WebSocketSession session;
            session = GetSessionByID(sn);
            if (session == null)
        ...
        public static bool GetSessionBySN(string sn)
        {
            WebSocketSession session = GetSessionByID(sn);
