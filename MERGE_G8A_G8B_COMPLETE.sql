-- ============================================
-- COMPLETE MERGE: G8A and G8B into G8
-- This handles students AND all subject tables
-- ============================================
...
DECLARE
    ...
    student RECORD;
BEGIN
    SELECT COALESCE(MAX(id), 0) INTO max_id FROM classes_schema."G8A";
    
    FOR student IN SELECT * FROM classes_schema."G8B" LOOP
        max_id := max_id + 1;
...
-- Step 3: Merge all subject tables (g8a_term_X + g8b_term_X)
-- For each subject schema, merge term 1 and term 2
...
-- Step 4: Update all references to G8B -> G8A
UPDATE school_schema_points.class_teachers SET assigned_class = 'G8A' WHERE assigned_class = 'G8B';
UPDATE subjects_of_school_schema.teacher_assignments SET "className" = 'G8A' WHERE "className" = 'G8B';
UPDATE subjects_of_school_schema.mark_lists SET class_name = 'G8A' WHERE class_name = 'G8B';
UPDATE schedule_schema.class_subject_configs SET class_name = 'G8A' WHERE class_name = 'G8B';
UPDATE schedule_schema.teachers SET assigned_classes = array_replace(assigned_classes, 'G8B', 'G8A') WHERE 'G8B' = ANY(assigned_classes);
UPDATE evaluation_book_schema.teacher_class_assignments SET class_name = 'G8A' WHERE class_name = 'G8B';
