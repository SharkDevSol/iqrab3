...
const GuardianCommunications = ({ wards, guardianInfo }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (wardClasses.length > 0) {
          try {
            const assignmentsRes = await axios.get('https://iqrab2.skoolific.com/api/class-teacher/assignments');
        ...
        if (guardianUsername) {
          try {
            const contactsRes = await axios.get(`https://iqrab2.skoolific.com/api/chats/contacts/guardian/${guardianUsername}`);
        ...
        if (teacherMap.size === 0) {
          try {
            const allContactsRes = await axios.get('https://iqrab2.skoolific.com/api/chats/contacts');
        ...
        const requestsRes = await axios.get(`https://iqrab2.skoolific.com/api/chats/user/${guardianId}`);
    ...
    socket.current = io('https://iqrab2.skoolific.com');
  ...
  const handleQuickSend = async () => {
    ...
    setSending(true);
    try {
      const response = await axios.post('https://iqrab2.skoolific.com/api/chats/requests', {
        senderId: guardianId,
