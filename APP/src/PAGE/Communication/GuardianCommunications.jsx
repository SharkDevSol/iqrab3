    ...
    const fetchData = async () => {
      try {
        ...
        if (wardClasses.length > 0) {
          try {
            const assignmentsRes = await axios.get('http://localhost:5000/api/class-teacher/assignments');
        ...
        if (guardianUsername) {
          try {
            const contactsRes = await axios.get(`http://localhost:5000/api/chats/contacts/guardian/${guardianUsername}`);
        ...
        if (teacherMap.size === 0) {
          try {
            const allContactsRes = await axios.get('http://localhost:5000/api/chats/contacts');
        ...
        const requestsRes = await axios.get(`http://localhost:5000/api/chats/user/${guardianId}`);
