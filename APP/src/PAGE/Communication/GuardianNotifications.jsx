...
const GuardianNotifications = () => {
  ...
  const fetchStatus = async () => {
    try {
      ...
      const response = await axios.get('https://iqrab2.skoolific.com/api/guardian-notifications/status', {
  ...
  const fetchGuardians = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('https://iqrab2.skoolific.com/api/guardian-list/guardians', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSendAttendance = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://iqrab2.skoolific.com/api/guardian-notifications/send-attendance',
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSendPayments = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(
        'https://iqrab2.skoolific.com/api/guardian-notifications/send-payments',
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setMessage({ type: 'success', text: response.data.message });
  ...
  const handleTestEmail = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(
        'https://iqrab2.skoolific.com/api/guardian-notifications/test-email',
        { email: testEmail },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setMessage({ type: 'success', text: response.data.message });
  ...
  const handlePreview = async (type) => {
    ...
    try {
      ...
      const endpoint = type === 'attendance' 
        ? `https://iqrab2.skoolific.com/api/guardian-notifications/preview-attendance/${selectedGuardian}`
        : `https://iqrab2.skoolific.com/api/guardian-notifications/preview-payment/${selectedGuardian}`;
