  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const contactsRes = await axios.get('http://localhost:5000/api/chats/contacts/admin');
        dispatch(setContacts({ role: 'director', contacts: contactsRes.data }));

        // Fetch all requests for this director
        const requestsRes = await axios.get(`http://localhost:5000/api/chats/user/${directorId}`);
        ...
        try {
          const contactsRes = await axios.get('http://localhost:5000/api/chats/contacts');
          dispatch(setContacts({ role: 'director', contacts: contactsRes.data }));
        } catch (fallbackErr) {
