...
const EvaluationFormDisplay = () => {
  ...
  useEffect(() => {
    const fetchEvaluationData = async () => {
      try {
        ...
        const evaluationRes = await fetch(`https://iqrab2.skoolific.com/api/evaluations/${id}`);
        if (!evaluationRes.ok) {
        ...
        const studentsRes = await fetch(`https://iqrab2.skoolific.com/api/evaluations/${id}/students`);
        if (!studentsRes.ok) {
  ...
  const handleSubmitResponses = async () => {
    ...
    try {
      const response = await fetch(`https://iqrab2.skoolific.com/api/evaluations/${id}/responses`, {
        method: 'POST',
        headers: {
