...
const EvaluationFormDisplay = () => {
  ...
  useEffect(() => {
    const fetchEvaluationData = async () => {
      try {
        ...
        const evaluationRes = await fetch(`http://localhost:5000/api/evaluations/${id}`);
        if (!evaluationRes.ok) {
        ...
        const studentsRes = await fetch(`http://localhost:5000/api/evaluations/${id}/students`);
        if (!studentsRes.ok) {
  ...
  const handleSubmitResponses = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/evaluations/${id}/responses`, {
        method: 'POST',
        headers: {
