...
const StudentProfile = () => {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      const response = await axios.get(`https://iqrab2.skoolific.com/api/students/profile/${username}`);
  ...
  const fetchProfilePosts = useCallback(async (schoolId) => {
    try {
      const response = await axios.get(`https://iqrab2.skoolific.com/api/posts/profile/student/${schoolId}`);
  ...
  const fetchMarkList = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `https://iqrab2.skoolific.com/api/mark-list/student-marks/${student.school_id}/${encodeURIComponent(student.class)}`
      );
